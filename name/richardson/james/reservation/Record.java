...
public abstract class Record {
  ...
  /**
   * Return all the records that match the example provided.
   * 
   * @param exampleRecord
   * The example record that should be used for matching.
   * @return records
   * A list of records that matched the example provided.
   */
  protected static final List<? extends Record> findAll(final Record example) {
    Record.logger.debug("Attempting to return records matching an example.");
    Record.logger.debug(example.toString());
    final ExampleExpression expression = Database.getInstance().getExpressionFactory().exampleLike(example, true, LikeType.EQUAL_TO);
    return Database.getInstance().find(example.getClass()).where().add(expression).findList();
  ...
  /**
   * Return the first record that matches the example provided.
   * 
   * @param exampleRecord
   * The example record that should be used for matching.
   * @return record
   * The record found or null if no such record exists.
   */
  protected static Record findFirst(final Record example) {
    Record.logger.debug("Attempting to the return the first record matching an example.");
    Record.logger.debug(example.toString());
    final ExampleExpression expression = Database.getInstance().getExpressionFactory().exampleLike(example, true, LikeType.EQUAL_TO);
    final List<? extends Record> records = Database.getInstance().find(example.getClass()).where().add(expression).findList();
    if (records.size() == 0)
      return null;
    else
      return Database.getInstance().find(example.getClass()).where().add(expression).findList().get(0);
  }
