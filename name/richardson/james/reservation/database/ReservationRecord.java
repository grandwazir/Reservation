...
public class ReservationRecord {
  ...
  protected static ReservationRecord find(String playerName) {
    ...
    ExampleExpression expression = ReservationRecord.database
        .getExpressionFactory().exampleLike(example, true, LikeType.EQUAL_TO);
    // find and return all bans that match the expression
    return ReservationRecord.database.find(ReservationRecord.class).where()
        .add(expression).findList().get(0);
  }
